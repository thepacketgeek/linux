...
struct NetconsoleTarget {
    ...
    #[cfg(CONFIG_NETCONSOLE_DYNAMIC)]
    #[pin]
    userdata_cache: SpinLock<dynamic::UserdataCache>,
...
pub(crate) struct ConfigState {
    ...
    /// Device name.
    pub(crate) dev_name: [u8; IFNAMSIZ],
    /// Local port.
...
impl NetconsoleTarget {
    ...
    fn new_with_config(cfg: Option<ParsedConfig>) -> impl PinInit<Self, Error> {
        ...
        .pin_chain(|_this: Pin<&mut Self>| {
            // Set up the userdata's back-pointer to the target
            #[cfg(CONFIG_NETCONSOLE_DYNAMIC)]
            {
                // SAFETY: _this is valid and pinned.
                let target_ptr = _this.as_ref().get_ref() as *const Self;
                ...
                unsafe {
                    let userdata_group_ptr = &_this.as_ref().get_ref().userdata_group;
                    let userdata_ptr =
    ...
    fn send_msg(&self, msg: &[u8]) {
        ...
        if let Some(ref np) = *np_guard {
            ...
            #[cfg(CONFIG_NETCONSOLE_DYNAMIC)]
            {
                ...
                if fields != 0 {
                    let mut sysdata_buf = [0u8; 256 * dynamic::MAX_SYSDATA_ITEMS];
    ...
    fn prepare_sysdata(&self, fields: u32, buf: &mut [u8]) -> usize {
        ...
        if fields & dynamic::SYSDATA_TASKNAME != 0 {
            ...
            let comm_bytes: &[u8] =
                unsafe { core::slice::from_raw_parts(comm.as_ptr().cast(), comm_len) };
            offset += format_sysdata_bytes(&mut buf[offset..], b"taskname", comm_bytes);
        }
        if fields & dynamic::SYSDATA_RELEASE != 0 {
            ...
            let release = unsafe {
                let ptr = &raw const kernel::bindings::init_uts_ns;
                &(*ptr).name.release
            };
            let rel_len = release
                .iter()
            ...
            let rel_bytes: &[u8] =
                unsafe { core::slice::from_raw_parts(release.as_ptr().cast(), rel_len) };
            offset += format_sysdata_bytes(&mut buf[offset..], b"release", rel_bytes);
        }
        if fields & dynamic::SYSDATA_MSGID != 0 {
...
struct NetconsoleModule {
    ...
    #[cfg(not(CONFIG_NETCONSOLE_DYNAMIC))]
    cmdline_target: Option<Arc<NetconsoleTarget>>,
    /// The configfs subsystem.
    #[cfg(CONFIG_NETCONSOLE_DYNAMIC)]
    #[pin]
    config: configfs::Subsystem<dynamic::NetconsoleSubsys>,
}
                ...
                let target: Arc<configfs::Group<NetconsoleTarget>> = Arc::pin_init(
                    ...
                    .pin_chain(|group: Pin<&mut configfs::Group<NetconsoleTarget>>| {
                        ...
                        unsafe {
                            ...
                            let child_group =
                                configfs::Group::<dynamic::Userdata>::group(&target_data.userdata_group)
                                    .cast_mut();
...
impl kernel::InPlaceModule for NetconsoleModule {
    fn init(_module: &'static ThisModule) -> impl PinInit<Self, Error> {
        ...
        .pin_chain(move |this: Pin<&mut Self>| {
            // If cmdline config was provided, create and enable the target
            if let Some(cfg) = cmdline_cfg {
                pr_info!("netconsole: creating cmdline target\n");

                let target: Arc<NetconsoleTarget> = Arc::pin_init(
                    NetconsoleTarget::new_with_config(Some(cfg)),
                    flags::GFP_KERNEL,
                )?;
                target.enable()?;
                ...
                // SAFETY: The target is valid and will remain valid while stored in the module.
                unsafe {
                    enabled_targets().add(core::ptr::from_ref(target.as_ref()))?;
                }
...
impl PinnedDrop for NetconsoleModule {
    fn drop(self: Pin<&mut Self>) {
        ...
        if let Some(ref target) = self.cmdline_target {
            // Only clean up enabled state if it was actually enabled
            let was_enabled = *target.enabled.lock();
            if was_enabled {
                // Remove from enabled targets list
                enabled_targets().remove(core::ptr::from_ref(target.as_ref()));

                // Disable the target
                *target.enabled.lock() = false;
                *target.netpoll.lock() = None;
...
mod dynamic {
    ...
    use kernel::configfs::{self, configfs_attrs};
    use kernel::page::PAGE_SIZE;
    use kernel::sync::ArcBorrow;

    /// Sysdata feature bitflags (matching C's `enum sysdata_feature`).
    pub(crate) const SYSDATA_CPU_NR: u32 = 1 << 0;
    pub(crate) const SYSDATA_TASKNAME: u32 = 1 << 1;
    pub(crate) const SYSDATA_RELEASE: u32 = 1 << 2;
    pub(crate) const SYSDATA_MSGID: u32 = 1 << 3;
    pub(crate) const MAX_SYSDATA_ITEMS: usize = 4;

    /// Maximum length of a userdata value.
    pub(crate) const MAX_EXTRADATA_VALUE_LEN: usize = 200;
    /// Maximum length of a userdata key name.
    pub(crate) const MAX_EXTRADATA_NAME_LEN: usize = 53;
    /// Maximum number of userdata entries per target.
    pub(crate) const MAX_USERDATA_ITEMS: usize = 256;

    pub(crate) static CMDLINE_CONFIGS: CmdlineConfigHolder = CmdlineConfigHolder::new();

    /// Netconsole subsystem data for configfs.
    #[pin_data]
    pub(crate) struct NetconsoleSubsys;
    ...
    pub(crate) struct UserdataCache {
        /// Pre-formatted " key=value\n" strings.
        pub(crate) data: Option<KVec<u8>>,
    ...
    impl UserdataEntry {
        ...
        pub(crate) fn new(key: &CStr, parent: *const Userdata) -> impl PinInit<Self, Error> {
    ...
    pub(crate) struct UserdataValue {
        /// The value data.
    ...
    impl Userdata {
        ...
        pub(crate) unsafe fn set_target(&self, target: *const NetconsoleTarget) {
    ...
    pub(crate) struct CmdlineConfigHolder {
    ...
    pub(crate) fn userdata_item_type(
    ) -> &'static configfs::ItemType<configfs::Group<Userdata>, Userdata> {
        configfs_attrs! {
    ...
    impl configfs::AttributeOperations<0> for NetconsoleTarget {
        ...
        fn store(container: &NetconsoleTarget, page: &[u8]) -> Result {
            ...
            if val {
                ...
                let np: Pin<KBox<Netpoll>> =
                    KBox::try_pin_init(builder.setup(), flags::GFP_KERNEL)?;
                ...
                let add_result =
                    unsafe { enabled_targets().add(container as *const NetconsoleTarget) };
    ...
    pub(crate) fn parse_bool(page: &[u8]) -> Result<bool> {
        let trimmed = trim_newline(page);
        if trimmed.is_empty() {
            return Err(EINVAL);
        }
        match trimmed {
            ...
            b"0" | b"n" | b"N" | b"no" | b"No" | b"NO" | b"false" | b"False" | b"FALSE" => {
                Ok(false)
            }
            _ => Err(EINVAL),
        }
    ...
    pub(crate) fn parse_u16(page: &[u8]) -> Result<u16> {
        let trimmed = trim_newline(page);
        if trimmed.is_empty() {
            return Err(EINVAL);
        }
    ...
    pub(crate) fn format_u16(buf: &mut [u8; 16], val: u16) -> &[u8] {
        let mut n = val;
        let mut i = buf.len();
        if n == 0 {
            buf[buf.len() - 1] = b'0';
    ...
    pub(crate) fn format_u64(buf: &mut [u8; 32], val: u64) -> &[u8] {
        let mut n = val;
        let mut i = buf.len();
        if n == 0 {
            buf[buf.len() - 1] = b'0';
    ...
    pub(crate) fn format_u32(buf: &mut [u8; 16], val: u32) -> &[u8] {
        let mut n = val;
        let mut i = buf.len();
        if n == 0 {
            buf[buf.len() - 1] = b'0';
    ...
    /// Returns the number of bytes written.
    pub(crate) fn format_sysdata_u32(buf: &mut [u8], key: &[u8], val: u32) -> usize {
        let mut tmp = [0u8; 16];
    ...
    /// Returns the number of bytes written.
    pub(crate) fn format_sysdata_bytes(buf: &mut [u8], key: &[u8], val: &[u8]) -> usize {
...
impl ParsedConfig {
    ...
    fn print_banner(&self) {
        ...
        let dev_name_len = strlen(&self.dev_name);
        if dev_name_len > 0 {
            if let Ok(s) = core::str::from_utf8(&self.dev_name[..dev_name_len]) {
